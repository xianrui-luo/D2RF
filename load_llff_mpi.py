...
def _minify(basedir, factors=[], resolutions=[]):
    ...
    ##modified for NVDS
    imgdir = os.path.join(basedir, 'images_bokeh')
...
def _load_data(basedir, start_frame, end_frame, 
    ...
    img0 = [os.path.join(basedir, 'images_bokeh', f) for f in sorted(os.listdir(os.path.join(basedir, 'images_bokeh'))) \
    ...
    disp_dir = os.path.join(basedir, 'dpt')  
    ...
    bg_mask_dir = os.path.join(basedir, 'background_mask')
    bg_maskfiles = [os.path.join(bg_mask_dir, f) \
                for f in sorted(os.listdir(bg_mask_dir)) if f.endswith('png') or f.endswith('jpg')]
    # maskfiles = maskfiles[start_frame:end_frame]

    bg_masks = [cv2.resize(imread(f)/255., (imgs.shape[1], imgs.shape[0]), 
                        interpolation=cv2.INTER_NEAREST) for f in bg_maskfiles]
    bg_masks = np.stack(bg_masks, -1)  
    bg_masks = np.float32(bg_masks > 1e-3)
    ...
    return poses, bds, imgs, disp, masks, motion_coords,bg_masks
...
def load_llff_data(basedir, start_frame, end_frame, 
    ...
    poses, bds, imgs, disp, masks, motion_coords,bg_masks = _load_data(basedir, 
    ...
    bg_masks = np.moveaxis(bg_masks, -1, 0).astype(np.float32)
    ...
    bg_masks = bg_masks.astype(np.float32)

    return images, disp, masks, poses, bds,\
        render_poses, c2w, motion_coords,bg_masks
