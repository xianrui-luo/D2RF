...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=True,activation=False,N_image=1):
        """ 
        """
...
def read_optical_flow_NVDS(basedir, img_i, start_frame, fwd):
    import os
    flow_dir = os.path.join(basedir, 'flow_i1')

    if fwd:

      fwd_flow_path = os.path.join(flow_dir,'%06d_left_fwd.npz'%(start_frame + img_i))
      fwd_flow_path=str(fwd_flow_path)
      ...
      bwd_flow_path = os.path.join(flow_dir, 
                                  '%06d_left_bwd.npz'%(start_frame + img_i))
...
def read_optical_flow(basedir, img_i, start_frame, fwd):
    ...
    if fwd:
      fwd_flow_path = os.path.join(flow_dir,'%03d_fwd.npz'%(start_frame + img_i))
      fwd_flow_path=str(fwd_flow_path)
      fwd_data = np.load(fwd_flow_path)#, (w, h))
      fwd_flow, fwd_mask = fwd_data['flow'], fwd_data['mask']
      fwd_mask = np.float32(fwd_mask)  
